...
const Signatures = require("./signature-list")
...
StateViewer.prototype.render = function () {
  ...
  return h(".state-viewer", [
    h(
      ...
      [h("p", `MetaMask Version ${version}`), h("div", [h("span", "Current Account: "), h(Address, { address: selectedAddress }), h("p", "Browser: " + browser)]), anyLost ? this.renderLost() : null, this.renderBalance()],
    ),

    h(Transactions, { transactions, isMobile: false }),
    h(Signatures, { signatures, isMobile: false }),
  ])
}
...
StateViewer.prototype.renderBalance = function () {
  ...
  const { parsedFile } = props
  const { metamask } = parsedFile
  const { selectedAddress, accountsByChainId } = metamask
